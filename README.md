...
2. Describe the different phases of the component lifecycle.
  ...
  Answer: 
          1. The Birth/Mounting Phase
            Is come after the initailization is completed. Mounting occure when the component is placed on the DOM container and the component is rendered on a webpage. the mounting phase has 2 methods:
            componentWillMount(): This method is called just before the compoentn is place on DOM.
            componentDidmount(): This method is called after the component is placed on DOM. TThe function get called after the render fucntion is executed for the first time.
          2. Growth/ Updation Phase
            Is a phase where the state and properties populated at the time initialization are changed if required after some user events. Will has invoked functions as below:
            componentWillReceiveProps()
            shouldComponentUpdate()
            componentWillUpdate()
            componentDidUpdate()
          3. Death/Unmounting Phase
            This phase is the last phase in component lifecycle detached from the DOM container: 
            Method for this is: 
            componentWillUnmount(): This method involed before a component is finally detached from theDOM and called when a component is fully removed from the page and show the end of lifecycle.



3. Demonstrate an understanding of class component lifecycle methods.

    Answer: componentWillMount(): This method is called just before the compoentn is place on DOM.
            componentDidmount(): This method is called after the component is placed on DOM. TThe function get 
...
4. Define stateful logic.
    ...
    Answer: Is logic that is built into a component. It can be a function handle a click event or can be function that set toggle state or function that formats data before get it displayed.

5. Describe how to test a React component with React Testing Library.

    Answer: The framework we use for testing is 3a, arrage, act and, assert. First, we 'arrange' our test by setting up our code such that it can be tested. Then, we 'act' calling a method or function that returns a result of interest to our test. Last, we 'assert' if our expected return matched the actual return.
